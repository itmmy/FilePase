...
public class FileTool {
	...
	public static void makeTestFile(Configurer conf) {
		...
		try {
			...
			Long lineNumer = Long.parseLong(conf.getConfigContext().get(AllParameter.LINENUMS));
			if (lineNumer<=500000) {
				String temp = makeData(lineNumer," ");
				writer.write(temp);
				writer.close();
			}else {
				int times = (int)(lineNumer%500000==0?lineNumer/500000:lineNumer/500000+1);
				StringBuffer sb = new StringBuffer();
				for (int i = 0; i < times; i++) {
					sb.replace(0,Integer.MAX_VALUE,makeData(500000," "));
	...
	public static Long gloubCount = 0l;
	/**
	 * 根据配置文件中的指定条数创建测试数据
	 * @param lineNums 配置文件中指定的数据条数 即：一条就是一行
	 * @return 所有数据放置在一起的一个完整字符串
	 */
	private static String makeData(long lineNums,String lineSeparators) {
		...
		for (int i = 0; i < lineNums; i++) {
			buffer.append(phonenumbers[random.nextInt(10)]+lineSeparators+random.nextInt(512)+lineSeparators+random.nextInt(512)+System.lineSeparator());
//			if (i%temp==0) {
...
//			}
		...
		Long stop = System.currentTimeMillis();
		//输出创建测试数据所耗费的总时间
		System.out.println("生成第"+ ++gloubCount +"个"+lineNums+"条测试数据一共耗时："+(stop-start)+"毫秒");
